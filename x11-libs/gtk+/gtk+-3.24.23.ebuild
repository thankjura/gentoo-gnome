...
inherit flag-o-matic gnome.org gnome2-utils meson multilib virtualx multilib-minimal xdg
...
LICENSE="LGPL-2+"
SLOT="3"
IUSE="aqua broadway cloudprint colord cups examples gtk-doc +introspection test vim-syntax wayland +X xinerama tracker"
REQUIRED_USE="
	|| ( aqua wayland X )
	xinerama? ( X )
"
...
src_prepare() {
	...
	eapply "${FILESDIR}"/${PN}-3.24.23-update-icon-cache.patch
...
multilib_src_configure() {
	...
	for u in cloudprint cups; do
		if use ${u}; then
			PRINT_BACKENDS=${PRINT_BACKENDS},${u}
		fi
	done

	if use cups; then
		if [[ -n "${PRINT_BACKENDS}" ]] ; then
	...
	local emesonargs=(
		...
		-Dprofiler=false
		-Dtracker3=$(multilib_native_usex tracker true false)
...
multilib_src_install() {
	meson_src_install
	keepdir /usr/$(get_libdir)/gtk-3.0/3.0.0
}
...
multilib_src_install_all() {
	...
	einstalldocs
	rm "${ED}/usr/share/man/man1/gtk-update-icon-cache.1" || die
}
