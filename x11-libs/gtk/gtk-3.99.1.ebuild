...
inherit gnome.org gnome2-utils meson multilib virtualx multilib-minimal xdg
...
multilib_src_configure() {
	local emesonargs=(
		...
		-Dprint=file,lpr$(usex cups ,cups '')$(usex cloudprint ,cloudprint '')
	)
...
multilib_src_test() {
	...
	if use demos; then
		"${EROOT}${GLIB_COMPILE_SCHEMAS}" --allow-any-name "${S}/demos" || die
		GSETTINGS_SCHEMA_DIR="${S}/demos" virtx meson_src_test
	fi
}
